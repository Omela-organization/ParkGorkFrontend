...
import EcoProblemMap from '@/components/eco/EcoProblemMap.vue'
import EcoFilterBar from '@/components/eco/EcoFilterBar.vue'
...
const statusOptions = computed(() =>
  (statuses.value || []).map(s => ({ id: s.id, label: s.text ?? s.name ?? String(s.id) })),
)
const typeOptions = computed(() =>
  (types.value || []).map(t => ({ id: t.id, label: t.text ?? t.name ?? String(t.id) })),
)
...
/* НОРМАЛЬНЫЙ ПОРЯДОК: latitude=55.x (lat), longitude=37.x (lon) */
...
function inMapView(p) {
  if (!filters.value.onlyInView) return true
  const bounds = mapRef.value?.getBounds?.()
  if (!bounds) return true
  const [lat, lon] = coordsPair(p)
  return (
    lat >= bounds.sw.lat && lat <= bounds.ne.lat &&
    lon >= bounds.sw.lon && lon <= bounds.ne.lon
  )
}
...
const filteredSorted = computed(() => {
  ...
  let arr = problems.value.filter((p) => {
    ...
    if (f.statuses.length && !f.statuses.includes(p.status_id)) return false
    if (f.types.length    && !f.types.includes(p.type_incident_id)) return false

    if (f.seen !== 'all') {
      const want = f.seen === 'true'
      if (!!p.is_seen !== want) return false
    }

    if (!inMapView(p)) return false
    return true
  })
  ...
  arr.sort((a, b) => {
    ...
    if (key === 'created_at') return (new Date(va) - new Date(vb)) * dir
    if (key === 'is_seen')    return ((+!!va) - (+!!vb)) * dir
    if (key === 'status_id')  return ((statusLabelById.value[va]||'').localeCompare(statusLabelById.value[vb]||'')) * dir
    if (key === 'type_incident_id')
      return ((typeLabelById.value[va]||'').localeCompare(typeLabelById.value[vb]||'')) * dir
...
async function openPhotos(p) {
  ...
  try {
    ...
    selectedPhotos.value =
      detailsCache.value.get(p.id)?.files?.map((f) => f.storage?.url).filter(Boolean) || []
  } finally {
...
function focusOnMap(p) { mapRef.value?.focusOn(p) }
function onRowClick(p) { focusOnMap(p) }
...
function cancelEditRow() { editRowId.value = null; draft.value = null }
async function saveEditRow() {
  ...
  const payload = {
    ...
    latitude: Number.isFinite(lat) ? String(lat) : undefined,   // широта
    longitude: Number.isFinite(lon) ? String(lon) : undefined,  // долгота
  }
  try {
...
function buildStaticMapUrl({ lat, lon, zoom = 17, size = '720,420' }) {
  const params = new URLSearchParams({
    lang: 'ru_RU', l: 'map', z: String(zoom), size,
...
<template>
  <section class="space-y-4">
        ...
        >
          ...
          <td class="px-3 py-2">
            <template v-if="isAdmin && editRowId === p.id">
              <select v-model="draft.type_incident_id"
          ...
          <td class="px-3 py-2">
            <template v-if="isAdmin && editRowId === p.id">
              <div class="flex gap-2">
                ...
                <input v-model="draft.lon"
                       class="text-sm bg-white border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-200 w-32"
                       placeholder="Долгота (37.x)" />
              </div>
            </template>
            <template v-else>
              <button class="font-mono text-blue-600 hover:underline"
          ...
          <td class="px-3 py-2">
            <button @click.stop="openPhotos(p)" class="px-2 py-1 rounded bg-gray-100 hover:bg-gray-200">
              Показать
            </button>
          </td>
          ...
          <td class="px-3 py-2">
            <template v-if="isAdmin && editRowId === p.id">
              <select v-model="draft.status_id"
                      class="text-sm bg-transparent border border-gray-200 rounded px-2 py-1 focus:outline-none focus:ring-0 w-44">
          ...
          <td class="px-3 py-2">
            ...
            <template v-else>
              <select :value="p.is_seen ? 'true' : 'false'"
                      :disabled="savingInline.has(p.id)"
                      class="text-sm bg-transparent border border-gray-200 rounded px-2 py-1 focus:outline-none focus:ring-0 w-28"
          ...
          <td class="px-3 py-2">
            <div class="flex items-center gap-2">
              <button @click.stop="printReport(p)" class="px-2 py-1 rounded bg-gray-100 hover:bg-gray-200">
                Скачать PDF-отчёт
              </button>

              <template v-if="isAdmin">
                <template v-if="editRowId === p.id">
                  <button @click.stop="saveEditRow" class="px-2 py-1 rounded bg-emerald-500 text-white hover:bg-emerald-600">💾</button>
                  <button @click.stop="cancelEditRow" class="px-2 py-1 rounded bg-gray-300 hover:bg-gray-400">✖️</button>
                </template>
                <template v-else>
                  <button @click.stop="startEditRow(p)" class="px-2 py-1 rounded bg-emerald-400 text-white hover:bg-emerald-500">✏️</button>
                  <button @click.stop="removeRow(p.id)" class="px-2 py-1 rounded bg-red-500 text-white hover:bg-red-600">🗑</button>
                </template>
              </template>
            </div>
          </td>
    ...
    <!-- модалка с фото -->
    <div v-if="photosOpen" class="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50" @click="photosOpen = false">
      <div class="bg-white max-w-5xl w-full rounded-xl p-4 space-y-3" @click.stop>
        <div class="flex items-center justify-between">
          ...
          <button class="text-sm text-gray-500 hover:text-black" @click="photosOpen = false">Закрыть</button>
        </div>
        ...
        <div v-else class="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
          <a v-for="(url, i) in selectedPhotos" :key="i" :href="url" target="_blank" rel="noopener" class="block">
    ...
    <div class="print:block hidden">
      <article v-if="printingProblem" class="p-6">
        ...
        <img v-if="printMapUrl" :src="printMapUrl" id="printMapImg" alt="Карта инцидента" class="w-full rounded-lg border mb-4" />
    ...
    <!-- тост -->
    <div v-if="toast.show" class="fixed bottom-4 right-4 z-50 rounded-lg bg-black/80 text-white px-3 py-2 text-sm shadow">
      {{ toast.text }}
    </div>
  </section>
</template>
